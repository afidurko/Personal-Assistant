...
def main() -> int:
    ...
    batches = [(w, base + (1 if w < rem else 0), args.seed + w * 17) for w in range(workers)]
    batches = [b for b in batches if b[1] > 0]

    print(
        f"Aaron voice gate fuzz: n={args.n:,} workers={workers} seed={args.seed}",
        flush=True,
    )
    t0 = time.perf_counter()
    results = []
    ...
    report = {
        ...
        "ok": failed == 0,
        "backend": "hash_dev",
        "sampler": "inline_modular_plus_0.01pct_full_gate",
