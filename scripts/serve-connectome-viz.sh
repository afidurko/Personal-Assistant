#!/usr/bin/env bash
# Serve Cam connectome live visualization from repo root (vault + config resolve; 3D DTI cortex by default)
set -euo pipefail
...
echo "  flat  http://127.0.0.1:${PORT}/visualizations/connectome/flat.html"
echo "  tip   Glass HUD = near-clear shell; serve MUST be repo root for live feed"
echo "  or    npm run dev for full home (Cam + cortex)"
echo "Ctrl+C to stop"
