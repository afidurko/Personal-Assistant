...
def main() -> int:
    ...
    if hf.get("role") not in {"multi_node_gpu_training", "gpu_train"} and hf.get("sense") != "sense.train.higgsfield":
        errors.append("higgsfield.json must be GPU training, not Speak clips")
    if hf.get("presence") is True:
        errors.append("higgsfield.json must not mark presence=true")
    if hf.get("motor") != "motor.higgsfield":
        errors.append("higgsfield.json motor must remain motor.higgsfield (train)")
