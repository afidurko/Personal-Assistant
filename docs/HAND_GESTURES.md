...
Aaron's starter repo: the link did not come through with the request. A slot is
reserved in `config/integrations/hand-gestures.json` (`aaron_repos.slots`);
every repo Aaron adds mounts as a submodule and is mapped onto the vocabulary's
`primitives.poses` / `primitives.motions` — the vocabulary stays the source of truth.
...
| Part | What it holds |
|---|---|
| `recognizer` | engine, canned labels, custom model path, engagement distance, frame rate, Aaron repo slot |
| `primitives.poses` / `primitives.motions` | the alphabet — 16 poses (8 canned, 5 landmark rules, 3 custom) and 18 motions |
...
| `gestures` | 40 bindings: `steps` (pose + motion + duration window) → `meaning` → `action` in `contexts`, with confidence, cooldown, priority, support, source |
| `learning` | who may teach, the teach flow, precedence, conflicts, feedback, forgetting, custom-model retrain |

`config/gestures/actions.json` is the catalog of 33 actions the gestures may
...
| **P4 — teach mode** | Voice-triggered `teach` context, landmark sample capture, replay confirm, Model Maker retrain job for custom poses (`three`, `four`, `ok_sign`) | PWA, `scripts/cam-gestures.py`, `data/gestures/` | new `.task` ships via `switch.cam_enhance` |
| **P5 — more eyes** | Pupil world camera as a second `sense.vision.gesture` source; Aaron's added repos mapped onto primitives | `integrations/pupil`, `integrations/<gesture-repo>` | `switch.pupil_vision` |
...
- `python3 scripts/gesture-check.py` — database integrity, connectome / Sentinel / registry wiring, eight canonical resolver demos (collapse, expand, iPhone → iPad handoff, identity hold, switch hold, stop-while-speaking, yes-in-prompt, two-palms hold).
...
- **Repo link** — the starter repo did not arrive; slot reserved, will map on arrival.
